...
\]
The left side is an exact time derivative:
\[
    \frac{d}{dt}\!\left[
        \tfrac{1}{2}\dot{\theta}^{\,2}
        - \frac{g}{L}\cos\theta
...
$k = \sin(\theta_0/2)$, the integral transforms into
\begin{equation}
    \boxed{\;
    T(\theta_0)
    = \frac{4}{\omega_0}\,K(k),
...
\textbf{Jacobi elliptic functions}---objects that most physicists
have heard of but few have used by hand.
...
To classify their stability, we linearise the system
...
time using the classical \textbf{Runge--Kutta method} (RK4).  The
...
require \texttt{numpy}, \texttt{matplotlib}, and \texttt{scipy}.
